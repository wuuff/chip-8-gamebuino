...
void chip8_execute(C8 * CH8){
    ...
    for(times = 0; times < 1; times++){
        ...
        CH8->opcode = ((uint8_t)memory_get(CH8,CH8->pc + 1));
        CH8->opcode |= (((uint16_t)memory_get(CH8,CH8->pc)) << 8);
            ...
            case 0xF000:
                switch(CH8->opcode & 0x00FF){
                    ...
                    case 0x0033: // FX33: Stores the Binary-coded decimal representation of VX, with the most significant of three digits at the address in I, the middle digit at I plus 1, and the least significant digit at I plus 2
                        ...
                        memory_set(CH8,CH8->I,CH8->V[(CH8->opcode & 0x0F00) >> 8] / 100);
                        memory_set(CH8,CH8->I+1,(CH8->V[(CH8->opcode & 0x0F00) >> 8] / 10) % 10);
                        memory_set(CH8,CH8->I+2, CH8->V[(CH8->opcode & 0x0F00) >> 8] % 10);
                        CH8->pc += 2;
                    break;
